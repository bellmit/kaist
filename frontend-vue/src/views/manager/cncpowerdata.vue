    ...
    <CCard>
      <v-app class="sr-card-app-title">
        <CCardHeader>
          ...
          <strong> KAIST CNC 전력 데이터 </strong>
        </CCardHeader>
        <CCardBody>
          <CNavbar
              expandable="md"
        ...
        <v-dialog v-model="sensorGraph.show" persistent max-width="1200px">
          <v-card >
            <div class="sensor-detail-container-2">
              <CCardHeader>
                <CIcon name="cil-justify-center"/>
                <strong> KAIST IoT 데이터 그래프(CNC 전력) </strong>
              </CCardHeader>
...
export default {
  ...
  methods: {
      ...
      axios({
        ...
        responseType: 'blob'
          ...
          .then(response => {
            ...
            const link = document.createElement('a')
            link.href = url
            var download_file_name = 'kaist_iot_' + item.file_names + ".csv"
    ...
    async get_cncpower_data(id,file_names){
      ...
      let msg = JSON.parse(data.objects[0].data_msg)
      // msg.forEach(v => v.date_time = moment(file_names.substring(0,4) + '-' + file_names.substring(4,6) + '-' + file_names.substring(6,8) + ' ' +  v.date_time).valueOf())
      var msg_f = msg.filter((v,index) => (index % 2) == 0)
      this.cncpower_data_objs = msg_f.map((v,index) => ({
        x: index,
        y: v.w
      }))
    },
  ...
  computed: {
    ...
    chart6Option () {
      ...
      if(this.cncpower_data_objs.length > 0) {
        obj.series[0].data = this.cncpower_data_objs
      }
      obj.series[0].name = 'datetime'
      return obj
    },
  },
  watch: {
